...
%   min_corr         - minimum correlation threshold (default: 0.65)
%   ignored_quantile - quantile of correlations to ignore (default: 0.1)
%   maxTol           - maximum tolerance for flagged ratio (default: 0.25)
%   overlap          - overlap fraction, e.g., 0.75 for 75% (default: 0.75)
%
% Outputs:
%   badChan          - logical vector of bad channels
%   flaggedRatio     - ratio of windows each channel was flagged

if nargin < 3 || isempty(min_corr),         min_corr         = 0.65; end
if nargin < 4 || isempty(ignored_quantile), ignored_quantile = 0.1;  end
if nargin < 5 || isempty(maxTol),           maxTol           = 0.25; end
