...
markers = DATA{:,contains(DATA.Properties.VariableNames, 'Marker')};  % column 29
marker_sample_idx = find(markers~=0);     % event latencies (using samples?)
% markers_timestamps = t(marker_idx);     % get corresponding timestamps
nEv = length(marker_sample_idx);
if nEv>0
    fprintf('%g event markers detected. Importing them... \n', nEv)
    % unique(markers)
    for iEv = 1:nEv
        EEG.event(iEv).type = num2str(markers(marker_sample_idx(iEv)));    % event names in character strings
        EEG.event(iEv).latency = marker_sample_idx(iEv);   % Events latency in samples
    end
    EEG = eeg_checkset(EEG,'eventconsistency');
    EEG = eeg_checkset(EEG);
    disp("")
