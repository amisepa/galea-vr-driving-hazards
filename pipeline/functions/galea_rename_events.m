...
if nEv > 0
    for iEv = 1:nEv
        ...
        elseif strcmpi(EEG.event(iEv).type, '6') && strcmpi(EEG.event(iEv-1).type, 'crash_start')
            EEG.event(iEv).type = 'tire_pop';      % Experiment trial tire pop
        elseif strcmpi(EEG.event(iEv).type, '6') && strcmpi(EEG.event(iEv-1).type, 'no_crash_start')
