...
3. The three companion toolboxes, cloned next to this repository (the layout
   `galea_set_paths.m` expects — one folder containing `galea/`,
...
4. **Node.js** (only if you want to rebuild the manuscript .docx; the `docx`
   package is already vendored in `manuscript/`).
5. **Configure paths once**: open `galea_set_paths.m` and set the four defaults
...
```matlab
analysis/make_figures.m               % all manuscript figures from results_final
```

Figure 1 is a vector schematic (`manuscript/figures/figure1_overview.svg`),
...
Then rebuild the manuscript itself (numbers are read from the result files at
build time, never transcribed, and the builder refuses to run if an analysis is
incomplete):

```bash
node manuscript/build_manuscript.js
node manuscript/build_supplementary.js
```

### Deviations you may see

- Your permuted cluster p-values will differ in the third decimal from the
  manuscript's (permutation seeds); the cluster structure should not.
